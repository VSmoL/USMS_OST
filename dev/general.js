...
function onYouTubeIframeAPIReady() {
    ...
    player = new YT.Player('player', {
        height: '100%',
        width: '100%',
        playerVars: { 'controls': 2, 'color':'red', 'showinfo':0, 'fs': 0, 'iv_load_policy': 3 },
        events: {
...
//    playlistContent.push("<img src='img/playlistBG.png'/>");
    ...
    playlistContent.push("<ul id='YTListUL' class='slidee grow'>");
    ...
    for (song in fullPlaylist){
        if (fullPlaylist[song].snippet.description != "This video is unavailable." && fullPlaylist[song].snippet.description != "This video is private."){
            ...
            playlistContent.push("<li>"+
                "<div id='YTvideo' data-listIndex="+song+" data-videoid='"+videoId+"'>"+songName+" "+themeSong+"<br />"+
                    "<span style='color: #EF5682;'>"+senderName[1]+"</span>"+
                "</div>"+
...
//    document.getElementById("YTplaylist2").innerHTML = playlistContent.join("");

    vidId = fullPlaylist[0].snippet.resourceId.videoId;

//    stroll.bind( '#YTplaylist2 ul', { live: true } );

    
    var $frame  = $('#YTplaylist');
    ...
    $("#YTplaylist ul li").click(function(){
        if(!isDragging){
            ...
            sly.slideTo(sly.getPos($(this)).center, [false]);
    ...
    if(!firstTimeList){
        $("#YTPlayer").height(window.innerHeight - $("#infoWindow").outerHeight() - $("#nowPlaying").outerHeight() - $("#infoTabs").outerHeight() - 0 - $("#actionBar").outerHeight());
        $("#YTplaylist").height(window.innerHeight - $("#YTPlayer").outerHeight() - $("#nowPlaying").outerHeight() - $("#infoTabs").outerHeight() - 20 - $("#actionBar").outerHeight());
//        $("#thumbListSongs").height(window.innerHeight - $("#YTPlayer").outerHeight() - $("#nowPlaying").outerHeight() - $("#infoTabs").outerHeight() - 50 - $("#actionBar").outerHeight());
        firstTimeList = true;
    }
}

function listFullPlaylist2(){
    console.log("listFullPlaylist2");
    var playlistContent = [];
...
//      backdrop: true
//    })
    changeTab(document.getElementById("addsongtab"));
    $("#ytLink").val('');
...
function editThemeHtml(themeNumber){
    $("#weeklyRandomNumber").html("This week theme: <u>" + weeklyTheme[themeNumber] + "</u><br><span class='tooltiptext'>"+weeklyThemeDesc[themeNumber]+"</span>");
...
//            $("#nowPlaying > marquee > span").html(playlist.items[0].snippet.title)
            $("#nowPlaying").html(playlist.items[0].snippet.title)
...
//        }
        checkListContent.push("<div id='thumbTable' style='width:100%; text-align: center; border-spacing: 0px; padding: 10px; font-size: 20px'>");
        checkListContent.push("<div class='thumbContent' style='border-bottom: solid 1px white;    padding-right: 18px;'><div>Name</div> <div>Weekly</div> <div>Theme</div></div>");
        checkListContent.push("<div id='thumbListSongs' class='thumbListArea' style='overflow-y: scroll;'>");
//        checkListContent.push("<tr> <th>Name</th> <th>Weekly</th> <th>Theme</th> </tr>");
        
        for (var x in userSongChecklist){
            var arrayContent = userSongChecklist[x].split("$");
            checkListContent.push('<div class="thumbContent">')
            checkListContent.push('<div>'+arrayContent[0]+'</div>')
            if(arrayContent[1] == 1){
                checkListContent.push('<div style="color:#0cc700">&#9733;</div>');
            }
            else{
                checkListContent.push('<div style="color:#f10000">&#9734;</div>');
            }
            if(arrayContent[2] == 1){
                checkListContent.push('<div style="color:#0cc700">&#9733;</div>');
            }
...
//            $("#thumbsUp span").html(arrayResponse[0] + " / 8");
            ...
            else if(arrayResponse[0] >= 5 && arrayResponse[0] < 8){
                $("#thumbsUpIcon").attr("src", "img/pokaaliGl.png");
                $("#thumbsUp").addClass("no-events");
                thumbGiven = true;
            }
            else if(arrayResponse[0] >= 8){
...
//    });
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            thumblistContent.push("<div id='thumbTable' style='width:100%; text-align: center; border-spacing: 0px; padding: 10px; font-size: 20px'>");
            thumblistContent.push("<div class='thumbContent' style='border-bottom: solid 1px white;    padding-right: 18px;'><div></div><div>Song name</div> <div>Sender</div> <div></div></div>");
            
            
//            thumblistContent.push("<table id='thumbTable' style='width:100%; text-align: center; border-spacing: 0px; padding: 10px; font-size: 20px'>");
//            thumblistContent.push('<col width="5%"><col width="70%"><col width="20%"><col width="5%">');
//            thumblistContent.push("<thead> <th></th> <th>Song name</th> <th>Sender</th> <th></th> </thead>");
            
            thumblistContent.push('<div id="thumbListSongs" class="thumbListArea" style="overflow-y: scroll;">');

            for (thumbsong in arrayResponse){
                
                var thumbData = arrayResponse[thumbsong].split("$");
                var thumbimage = "";
                thumblistContent.push('<div class="thumbContent">');
                thumblistContent.push('<div data-videoid='+thumbData[2]+'><i class="material-icons">&#xE039;</i></div>');
                thumblistContent.push('<div>'+thumbData[0]+'</div>');
                thumblistContent.push('<div>'+thumbData[3]+'</div>');
                ...
                else if(thumbData[1] >= 5 && thumbData[1] < 8){
                    thumbimage = "img/pokaaliGl.png";
                }
                else if(thumbData[1] >= 8){
                    thumbimage = "img/pokaaliDi.png";
                }
                
                thumblistContent.push('<div><img width="20px" src="'+thumbimage+'"></div>');
                thumblistContent.push('</div>');
            }
            ...
            $("#popupthumblist div div div:first-child").click(function(){
            ...
            $("#thumbListSongs").height(window.innerHeight - $("#YTPlayer").outerHeight() - $("#nowPlaying").outerHeight() - $("#infoTabs").outerHeight() - 50)
        }
        else if (xmlhttp.readyState == 4 && xmlhttp.status != 200){
            console.log("ei onnaa");
        }
...
function changeTab(element){
    ...
    var tabelement = $(element).attr("data-tab");
    $("#infoWindow > div").hide();
    $("#"+tabelement).show();
    
    $("#infoTabs > div").removeClass("active");
    $(element).addClass("active");
    

//    $("#infoTabs > div.active").css({
//        "background": "rgba("+activeTab+", "+activeTab+", "+activeTab+", 0.4)"
//    });
}
...
function changeTheme(element){
    ...
    if(themeDL == "dark"){
        rColor = $(element).attr("data-darkR");
        gColor = $(element).attr("data-darkG");
        bColor = $(element).attr("data-darkB");
    }
    else if(themeDL == "light"){
        rColor = $(element).attr("data-lightR");
        gColor = $(element).attr("data-lightG");
        bColor = $(element).attr("data-lightB");
    }
    
    $("body").css({
        "background": "rgba("+rColor+", "+gColor+", "+bColor+", 1)"
    });
    
    $(".handle").css({
        "background": "radial-gradient(ellipse at center, rgba("+rColor+", "+gColor+", "+bColor+", 1) 40%,rgba(95,220,141,0) 69%,rgba(125,185,232,0) 100%)"
    });
    $("hr").css({
        "border-color": "rgba("+rColor+", "+gColor+", "+bColor+", 0.8)",
    });
}
var themeDL = "dark";
function changeBodyBG(color){
    ...
    $("#infoTabs > div:not(.active)").css({
        "background": "rgba("+activeTab+", "+activeTab+", "+activeTab+", 0.2)"
    });
    $(".scrollbar").css({
        "background": "radial-gradient(ellipse at center, "+color+" 40%,rgba(95,220,141,0) 69%,rgba(125,185,232,0) 100%)",
        "border": "solid 7px "+color,
        "box-shadow": "inset 0px 0px 20px 0px "+color,
    });
